...
export default function TemplateCardRenderer({ card, onAction }: Props) {
  ...
  const renderBlock = (block: Record<string, any>, index: number) => {
    ...
    if (type === 'hero') {
      ...
      return (
        <div key={`hero-${index}`} className="rounded-2xl border border-[rgba(15,111,255,0.08)] bg-[rgba(240,247,255,0.46)] px-4 py-4">
          <div className="text-base font-semibold text-slate-800">{blockTitle || title}</div>
          {blockSummary && <div className="mt-1 text-sm leading-6 text-slate-500">{blockSummary}</div>}
        </div>
      )
    }
      ...
      return (
        <div key={`metric-${index}`} className="grid gap-3 md:grid-cols-3">
          {items.map((item, itemIndex) => {
            ...
            return (
              <div key={`metric-item-${itemIndex}`} className="rounded-2xl border border-[rgba(15,111,255,0.08)] bg-[rgba(240,247,255,0.58)] px-4 py-3">
      ...
      return (
        <div key={`kv-${index}`} className="space-y-2">
          {items.map((item, itemIndex) => {
            const row = isPlainObject(item) ? item : { value: item }
            return (
              <div key={`kv-item-${itemIndex}`} className="flex items-start justify-between gap-3 rounded-xl border border-[rgba(15,111,255,0.08)] bg-[rgba(240,247,255,0.46)] px-3 py-2">
      ...
      return (
        <div key={`list-${index}`} className="space-y-3">
          {items.map((item, itemIndex) => {
            ...
            return (
              <div key={`list-item-${itemIndex}`} className="rounded-2xl border border-[rgba(15,111,255,0.08)] bg-[rgba(240,247,255,0.46)] px-4 py-4">
    ...
    return (
      <div key={`fallback-${index}`} className="rounded-xl border border-[rgba(15,111,255,0.08)] bg-[rgba(240,247,255,0.46)] px-3 py-3 text-xs text-slate-600 whitespace-pre-wrap break-all">
        {displayScalar(value)}
      </div>
    )
  }

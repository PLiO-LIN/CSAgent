...
def _card_id(name: str, args: dict, suffix: str = "") -> str:
    ...
    prefix = "".join(ch if ch.isalnum() or ch in {"_", "-"} else "_" for ch in str(name or "card").strip()) or "card"
    ...
    raw = f"{prefix}|{safe_suffix}|{json.dumps(normalized_args, ensure_ascii=False, sort_keys=True)}"
...
def _collect_tools(loaded_skills: list[Skill], global_tool_names: list[str] | None = None) -> list[ToolDef]:
    ...
    if global_tool_names:
        for name in global_tool_names:
    ...
    for skill in loaded_skills:
        for tool_name in skill.global_tools:
...
def _confirmation_request_text(name: str, args: dict) -> str:
    compact_args = {key: value for key, value in (args or {}).items() if value not in (None, "", [], {})}
    details = json.dumps(compact_args, ensure_ascii=False, sort_keys=True) if compact_args else "无额外参数"
    return (
...
async def run(
    ...
    base_scope_token = set_runtime_scope(
        agent_id=selected_agent_id,
        global_tool_names=agent_runtime.global_tool_names,
        skill_names=agent_runtime.skill_names,
        active_skill_names=list((agent_state.get("skill_state") or {}).get("active_skills", [])),
    )
    ...
    step_scope_token = None
    try:
        while step < settings.max_steps:
            if step_scope_token is not None:
                reset_runtime_scope(step_scope_token)
            step_scope_token = set_runtime_scope(
                agent_id=selected_agent_id,
                global_tool_names=agent_runtime.global_tool_names,
                skill_names=agent_runtime.skill_names,
                active_skill_names=[s.name for s in loaded_skills],
            )
            ...
            if should_compact and not breaker_open:
                ...
                try:
                    summary = await compact(
                        ...
                        raw_messages,
                        agent_state=agent_state,
                        summary_prompt=agent_runtime.summary_prompt,
                    )
            ...
            skill_summaries = render_skill_catalog(active_skill_names=[s.name for s in loaded_skills])
            runtime_controls = {
                "context_budget": context_budget,
                "step": step,
            ...
            system = build_system(
                ...
                system_core_prompt=agent_runtime.system_core_prompt,
        ...
        if step_scope_token is not None:
            reset_runtime_scope(step_scope_token)
        reset_runtime_scope(base_scope_token)
